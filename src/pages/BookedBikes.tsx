// import React, { useState, useRef, useEffect } from "react"; // must be uncommented
...
// const BookedBikes: React.FC = () => {
//   const navigate = useNavigate();
//   const { bookings, deleteBooking } = useBookingStore();

//   const [vehicleFilter, setVehicleFilter] = useState<"All" | "Car" | "Auto" | "Bike">("All");
...
//       if (menuRef.current && !menuRef.current.contains(event.target as Node)) setShowDeleteMenu(false);
//       if (bookingMenuRef.current && !bookingMenuRef.current.contains(event.target as Node)) setShowBookingMenu(null);
//     };
...
//     switch (value) {
//       case "Car": navigate("/booked-cars"); break;
//       case "Auto": navigate("/booked-autos"); break;
//       case "Bike": navigate("/booked-bikes"); break;
//       default: navigate("/bookings");
//     }
//   };

//   const getStatusBadge = (status: string, onClick: () => void) => {
//     const base = "w-[200px] h-[50px] flex items-center justify-center rounded-lg font-semibold text-sm cursor-pointer transition";
//     switch (status) {
//       case "Booked": return <button onClick={onClick} className={`${base} bg-green-100 text-green-700 hover:bg-green-200`}>Booked</button>;
//       case "Cancelled": return <button onClick={onClick} className={`${base} bg-red-100 text-red-700 hover:bg-red-200`}>Cancelled</button>;
//       default: return <button onClick={onClick} className={`${base} bg-gray-100 text-gray-700`}>{status}</button>;
//     }
//   };

//   const filteredBookings = vehicleFilter === "All" ? bookings : bookings.filter(b => b.vehicleType === vehicleFilter);

//   const handleDeleteAll = () => {
//     if (!filteredBookings.length) return alert("No bookings to delete");
...
//         <h2 className="text-2xl font-semibold">Booked Bikes</h2>
...
//           >
//             <img src={booking.vehicleImage || Activa} alt={booking.vehicleName} className="w-32 h-32 object-cover rounded-lg" />
...
//               </div>
//               <div className="pt-2">{getStatusBadge(booking.status, () => {
