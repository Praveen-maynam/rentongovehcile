...
const ListCarPage = () => {
  ...
  // New state for brand selection and transmission dropdown
  ...
  const formFields = {
    vehicleDetails: [
      ...
      { 
        ...
        type: "select", 
        required: true,
        options: carData?.seatingCapacity || [],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.CarBrand || !formData.CarModel || !formData.CarNumber) {
      alert("Please fill in car brand, model, and number");
      return;
    }

    if (!formData.fuelType || !formData.transmissionType) {
    ...
    // Set CarName as combination of Brand + Model
    ...
    if (field.type === "customDropdown") {
      return (
        <div className="relative">
          <button
            type="button"
            onClick={() => setTransmissionDropdownOpen(!transmissionDropdownOpen)}
            className={baseClasses + " flex items-center justify-between"}
          >
            <span className={displayValue ? "text-gray-900" : "text-gray-400"}>
              {displayValue || "Select Transmission"}
            </span>
            <span className={`transition-transform ${transmissionDropdownOpen ? 'rotate-180' : ''}`}>▼</span>
          </button>
