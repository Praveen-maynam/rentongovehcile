...
const Nearbybikes: React.FC = () => {
  ...
  const [bikes] = useState(vehicleData.filter((v) => v.type === "bike"));
  const [showFilter, setShowFilter] = useState(false); // ✅ state for modal
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6">
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            {/* Filter Button */}
            <button
              onClick={() => setShowFilter(true)} // ✅ opens filter modal
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {filteredBikes.length > 0 ? (
            filteredBikes.map((v) => <VehicleCard key={v.id} vehicle={v} />)
          ) : (
