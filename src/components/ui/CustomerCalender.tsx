...
//   // Fetch only owner blocks (vehicle availability)
//   const fetchOwnerBlocks = React.useCallback(async () => {
//     try {
//       setLoading(true);
//       const startDate = formatDate(new Date(currentMonth.getFullYear(), currentMonth.getMonth(), 1));
...
//       fetchOwnerBlocks();
//     }
//   }, [isOpen, VechileId, vechileType, fetchOwnerBlocks]);
...
//         } else {
//           showMsg("error", "Contact number is required");
//           setLoading(false);
//           return;
//         }
//       }

//       const contactName = propContactName || localStorage.getItem("userName") || localStorage.getItem("contactName") || "Customer";
//       const ownerId = propOwnerId || localStorage.getItem("vehicleOwnerId") || "";
...
//       if (response.ok) {
//         showMsg("success", "Booking request sent successfully!");
...
//       days.push(
//         <button key={d} className={cls} disabled={past || ownerBlocked} onClick={() => handleDateClick(date)}>
//           {d}
//           {ownerBlocked && <span className="absolute -top-1 -right-1 w-2 h-2 bg-red-500 rounded-full" />}
//         </button>
//       );
...
//           {/* Legend */}
//           <div className="flex flex-wrap gap-3 text-xs mb-4 pb-4 border-b">
...
//                   <span className="font-semibold">{formatDisplay(selectedStart)} at {startTime}</span>
//                 </div>
//                 <div className="flex justify-between items-center mb-2">
//                   <span className="text-sm text-gray-600">To:</span>
//                   <span className="font-semibold">{formatDisplay(selectedEnd)} at {endTime}</span>
//                 </div>
