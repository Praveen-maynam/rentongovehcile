...
//             records.push({
//               id: item._id || item.availabilityId || item.date,
//               from: item.date,
//               to: item.date,
...
export default function OwnerCalendar({
      ...
      if (data && Array.isArray(data.availability)) {
        ...
        data.availability.forEach((item: any) => {
          if (item.status === "Unavailable") {
            ...
            // Map each slot individually
            item.slots.forEach((slot: any) => {
              records.push({
                id: slot.id, // Use the unique slot id from API
                from: item.date,
                to: item.date,
  ...
  const isCustomerBooked = (date: Date) => customerBookedDates.includes(formatDate(date));
  ...
  const handleDateClick = (date: Date) => {
    if (isPast(date)) return;
    if (isCustomerBooked(date)) return showMsg("error", "Cannot modify customer bookings");
    ...
    if (!selectedStart) {
      ...
      if (date < selectedStart) return showMsg("error", "End date must be after start");
      setSelectedEnd(date);
    } else {
      setSelectedStart(date);
      setSelectedEnd(null);
  ...
  const handleCreateBlock = async () => {
    ...
    try {
      ...
      body.append("vechileType", vechileType);
      body.append("VechileId", VechileId);
      body.append("isNotAvailable", "false");
  ...
  const handleUpdateBlock = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showMsg("error", result.message || "Failed to update block");
      }
    } catch (error) {
      console.error("Error updating block:", error);
  ...
  const handleDeleteBlock = async (recordId: string) => {
    ...
    try {
      ...
      if (response.ok || response.status === 204) {
        ...
        const result = await response.json();
        showMsg("error", result.message || "Failed to delete block");
      }
    } catch (error) {
      console.error("Error deleting block:", error);
  ...
  const renderCalendar = () => {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      else if (custBooked) cls += "bg-orange-100 border-2 border-orange-400 text-orange-700 cursor-not-allowed";
      else if (ownerBlocked) cls += "bg-red-100 border-2 border-red-400 text-red-700 hover:bg-red-200 cursor-pointer";
      else if (selected) cls += "bg-blue-600 text-white border-2 border-blue-700";
