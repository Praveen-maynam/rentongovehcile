...
const PopupChat: React.FC<PopupChatProps> = ({ 
 ...
 // Fetch existing messages
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      try {
        console.log("📥 Fetching messages for veh:", vehicleId);
        const res = await fetch(`${apiUrl}/getMessages/${vehicleId}`);
    ...
    fetchMessages();
  }, [vehicleId, apiUrl, isOpen, useRealtime]);
  ...
  // ✅ IMPROVED: Extract senderId with better handling
  const getSenderId = (msg: ChatMessage): string => {
    ...
    // If senderId is an object (populated), extract the _id
  ...
  // ✅ FIXED: Determine if message is mine based on currentUserId comparison
  const isMyMessage = (msg: ChatMessage): boolean => {
    const msgSenderId = getSenderId(msg);
    
    // If we can't determine the sender, show on left (as received)
    if (!msgSenderId) {
      console.warn("⚠️ Cannot determine sender for message:", msg);
      return false;
    }
    
    // Normalize both IDs for comparison (trim whitespace, convert to lowercase)
    const normalizedMsgSenderId = msgSenderId.toLowerCase().trim();
    const normalizedCurrentUserId = currentUserId.toLowerCase().trim();
    
    // Simple comparison: if the message senderId matches currentUserId, it's mine
