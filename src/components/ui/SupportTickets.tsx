...
import { Ticket, Plus, ArrowLeft, Clock, CheckCircle, AlertCircle, XCircle } from 'lucide-react';
...
function SupportTicketsPage() {
  ...
  const fetchTickets = async () => {
    ...
    try {
      ...
      const result = await apiService.ticket.getTicketsByUser(userId);

      // Handle the response from apiClient which returns response.data directly
      // The result could be structured as: { tickets: [...] } or { data: [...] } or [...]
      const ticketsData = Array.isArray(result)
        ? result
        : (result as any)?.tickets || (result as any)?.data || [];
  ...
  const handleCreateTicket = () => window.location.href = '/raise-ticket';
  const handleBack = () => window.history.back();

  const getStatusIcon = (status: string) => {
    const s = status?.toLowerCase() || '';
    if (s === 'open' || s === 'pending') return <Clock size={20} className="text-orange-600" />;
    if (s === 'in progress' || s === 'inprogress') return <AlertCircle size={20} className="text-blue-600" />;
    if (s === 'resolved' || s === 'closed') return <CheckCircle size={20} className="text-green-600" />;
    if (s === 'cancelled') return <XCircle size={20} className="text-red-600" />;
    return <Ticket size={20} className="text-gray-600" />;
  };

  const getStatusColor = (status: string) => {
    const s = status?.toLowerCase() || '';
    if (s === 'open' || s === 'pending') return 'bg-orange-100 text-orange-700 border-orange-200';
    if (s === 'in progress' || s === 'inprogress') return 'bg-blue-100 text-blue-700 border-blue-200';
    if (s === 'resolved' || s === 'closed') return 'bg-green-100 text-green-700 border-green-200';
  ...
  const getPriorityColor = (priority: string) => {
    ...
    if (p === 'high' || p === 'urgent') return 'bg-red-50 text-red-700 border-red-200';
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        {error && (
          <div className="mb-6 p-4 bg-red-50 border-2 border-red-200 rounded-xl">
            <p className="text-red-700 font-medium">{error}</p>
            <button onClick={fetchTickets} className="mt-2 text-sm text-red-700 underline hover:text-red-800 font-medium">
              Try Again
            </button>
          </div>
        )}
          ...
          {!loading && tickets.length > 0 && (
            <div className="space-y-4">
              {tickets.map((ticket) => (
                <div
                  key={ticket._id}
                  className="border border-gray-200 p-5 rounded-lg hover:shadow-lg transition-shadow bg-white"
                >
                  ...
                  <div className="flex flex-wrap gap-2">
                    <div className={`inline-flex px-3 py-1.5 rounded-lg border text-sm font-medium ${getStatusColor(ticket.status)}`}>
                      {ticket.status}
                    </div>
                    <div className={`inline-flex px-3 py-1.5 rounded-lg border text-sm font-medium ${getPriorityColor(ticket.priority)}`}>
                      {ticket.priority} Priority
                    </div>
                    {ticket.vehicleType && (
                      <div className="inline-flex px-3 py-1.5 rounded-lg border text-sm font-medium bg-gray-50 text-gray-700 border-gray-200">
                        {ticket.vehicleType}
                      </div>
                    )}
                  </div>
