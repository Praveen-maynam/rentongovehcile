...
//       if (past) className += "bg-gray-100 text-gray-400 cursor-not-allowed";
//       else if (blocked)
//         className +=
...
//             >
//               {loading ? "Saving..." : "Confirm & Block Dates"}
//             </button>
//           </div>
//         </div>
...
//               {blockedSlots.length} blocked period(s) active.
//             </p>
//           </div>
//         )}
//       </div>
...
//         // Separate customer booked dates for visual distinction
//         const customerBooked = data.availability
//           .filter((item: any) => item.status === "Unavailable" && item.isCustomerBooking)
//           .map((item: any) => item.date);
...
//     // Check if this is a customer booking - owner cannot edit
//     if (slot.isCustomerBooking) {
//       showMessage("error", "Cannot edit customer bookings");
//       return;
//     }
...
//     const [fromHours, fromMins] = slot.fromTime.split(".");
//     const [toHours, toMins] = slot.toTime.split(".");
    
//     setStartTime(`${String(fromHours).padStart(2, "0")}:${fromMins || "00"}`);
//     setEndTime(`${String(toHours).padStart(2, "0")}:${toMins || "00"}`);
    
//     setShowOptionsMenu(false);
//     setShowDeleteListModal(false);
//     showMessage("info", "Editing mode: Select new dates and confirm");
//   };
...
//     // Check if this is a customer booking - owner cannot delete
//     if (slot.isCustomerBooking) {
//       showMessage("error", "Cannot delete customer bookings. Customer must cancel their booking.");
//       return;
//     }
...
//         className += "bg-gray-100 text-gray-400 cursor-not-allowed";
//       } else if (customerBooked) {
//         // Customer booked dates - orange/yellow color
//         className += "bg-orange-100 border-2 border-orange-500 text-orange-700 relative cursor-not-allowed";
//       } else if (blocked) {
//         // Owner blocked dates - red color
//         className += "bg-red-100 border-2 border-red-500 text-red-700 relative hover:bg-red-200";
...
//           disabled={past || customerBooked}
//           onClick={() => handleDateClick(date)}
//           title={customerBooked ? "Customer Booking" : blocked ? "Owner Blocked" : "Available"}
//         >
//           {d}
//           {customerBooked && (
//             <>
...
//         {/* BLOCKED SUMMARY */}
//         {(blockedSlots.length > 0 || customerBookedDates.length > 0) && (
//           <div className="mt-6 grid grid-cols-2 gap-4">
//             <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
//               <p className="font-bold text-green-800">
//                 {blockedSlots.length} Owner Blocked Period(s)
//               </p>
//             </div>
//             <div className="p-4 bg-orange-50 border border-orange-200 rounded-lg">
//               <p className="font-bold text-orange-800">
//                 {customerBookedDates.length} Customer Booking(s)
//               </p>
//             </div>
//           </div>
//         )}
...
//                       <div>
//                         <p className="font-semibold">
//                           {formatDateForDisplay(new Date(slot.fromDate.split("T")[0]))} - {formatDateForDisplay(new Date(slot.toDate.split("T")[0]))}
//                         </p>
//                         <p className="text-sm text-gray-600">
//                           Time: {slot.fromTime} - {slot.toTime}
//                         </p>
